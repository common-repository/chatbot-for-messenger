...
function qcwp_fb_custom_post_metabox_private_reply(){
	...
	$private_reply_condition = get_post_meta( $post->ID, 'private_reply_condition', true );
	$reply_condition = get_post_meta( $post->ID, 'reply_condition', true );
	$condition_value = get_post_meta( $post->ID, 'condition_value', true );
	$reply_text = get_post_meta( $post->ID, 'reply_text', true );
	...
	?>
		...
		<div class="wpfb_private_reply_conditional_area">
			...
			<input type="radio" name="wpfb_private_reply_condition" value="0" <?php echo ($private_reply_condition==0?'checked="checked"':''); ?> />
			<i><?php echo esc_html__('Reply anytime', 'wpfb'); ?></i>
			<input type="radio" name="wpfb_private_reply_condition" value="1" <?php echo ($private_reply_condition==1?'checked="checked"':''); ?> />
			...
			<?php if(!empty($reply_condition) or is_array($reply_condition)): ?>
				<?php foreach($reply_condition as $key=>$value): ?>
				<div class="wpfb_logic_elem">
					<span>Comment</span><select name="wpfb_condition[]">
					...
					</select><input type="text" name="wpfb_condition_value[]" value="<?php echo (isset($condition_value[$key]) && $condition_value[$key]!=''?$condition_value[$key]:''); ?>" /><a class="button button-secondary wpfb_logic_remove">Remove</a><br>
					Or
				</div>
				<?php endforeach; ?>
			<?php else: ?>
				<div class="wpfb_logic_elem">
					...
					</select><input type="text" name="wpfb_condition_value[]" value="" /><a class="button button-secondary wpfb_logic_remove">Remove</a><br>
					Or
				</div>
			<?php endif; ?>
			...
			<br>
			<a class="button button-secondary" id="wpfb_condition_add" <?php echo ($private_reply_condition==1?'style="display:inline-block"':'style="display:none"') ?>>Add New</a>
			
		</div>
		<div class="wpfb_reply_comment_area">
			<p>Reply Content:</p>
			<textarea name="wpfb_reply_text"><?php echo $reply_text; ?></textarea>
...
function qcwp_fb_custom_post_metabox_comment_reply(){
	...
	$comment_reply_is_condition = get_post_meta( $post->ID, 'comment_reply_is_condition', true );
	$comment_reply_condition = get_post_meta( $post->ID, 'comment_reply_condition', true );
	$comment_condition_value = get_post_meta( $post->ID, 'comment_condition_value', true );
	$comment_reply_text = get_post_meta( $post->ID, 'comment_reply_text', true );
	...
	?>
		...
		<div class="wpfb_private_reply_conditional_area">
			...
			<input type="radio" name="wpfb_comment_reply_condition" value="0" <?php echo ($comment_reply_is_condition==0?'checked="checked"':''); ?> />
			<i><?php echo esc_html__('Reply anytime', 'wpfb'); ?></i>
			<input type="radio" name="wpfb_comment_reply_condition" value="1" <?php echo ($comment_reply_is_condition==1?'checked="checked"':''); ?> />
			<i><?php echo esc_html__('Reply if', 'wpfb'); ?></i>
			...
			?>
				<?php foreach($comment_reply_condition as $key=>$value): ?>
				<div class="wpfb_logic_elem">
					<span>Comment</span><select name="wpfb_comment_condition[]">
						<option value="1" <?php echo ($value==1?'selected="selected"':''); ?>>is equal to</option>
						<option value="2" <?php echo ($value==2?'selected="selected"':''); ?>>contains</option>
					</select><input type="text" name="wpfb_comment_condition_value[]" value="<?php echo (isset($comment_condition_value[$key]) && $comment_condition_value[$key]!=''?$comment_condition_value[$key]:''); ?>" /><a class="button button-secondary wpfb_logic_remove">Remove</a><br>
					Or
				</div>
				<?php endforeach; ?>
			<?php else: ?>
				<div class="wpfb_logic_elem">
					<span>Comment</span><select name="wpfb_comment_condition[]">
						<option value="1">is equal to</option>
						<option value="2">contains</option>
					</select><input type="text" name="wpfb_comment_condition_value[]" value="" /><a class="button button-secondary wpfb_logic_remove">Remove</a><br>
					Or
				</div>
			<?php endif; ?>
			...
			<br>
			<a class="button button-secondary" id="wpfb_comment_condition_add" <?php echo ($comment_reply_is_condition==1?'style="display:inline-block"':'style="display:none"') ?>>Add New</a>
			
		</div>
		<div class="wpfb_reply_comment_area">
			<p>Reply Content:</p>
			<textarea name="wpfb_comment_reply_text"><?php echo $comment_reply_text; ?></textarea>
			<span>[sender_name], [sender_comment]</span>
		</div>
	<?php
...
function qcwpfb_save_fbposts_meta( $post_id ) {
	...
	if ( isset( $_POST['post_type'] ) && 'wpfbposts' === $_POST['post_type'] ) {
		...
		if(isset($_POST['wpfb_private_reply_condition'])){
			...
			add_post_meta($post_id, 'private_reply_condition', sanitize_text_field($_POST['wpfb_private_reply_condition']), true);
		}
		if(isset($_POST['wpfb_comment_reply_condition'])){
			delete_post_meta($post_id, 'comment_reply_is_condition');
			add_post_meta($post_id, 'comment_reply_is_condition', sanitize_text_field($_POST['wpfb_comment_reply_condition']), true);
		}
		...
		if(isset($_POST['wpfb_reply_text'])){
			...
			add_post_meta($post_id, 'reply_text', sanitize_textarea_field($_POST['wpfb_reply_text']), true);
		}
		
		if(isset($_POST['wpfb_comment_reply_text'])){
			delete_post_meta($post_id, 'comment_reply_text');
			add_post_meta($post_id, 'comment_reply_text', sanitize_textarea_field($_POST['wpfb_comment_reply_text']), true);
		}
